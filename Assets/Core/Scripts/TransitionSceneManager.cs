...
public class TransitionSceneManager : MonoBehaviour
{
    ...
    {
        if (!SceneIsLoaded(sceneName) || sceneName == _coreSceneName) yield break;
    ...
    {
        if (!SceneIsLoaded(sceneNumber) || SceneToString(sceneNumber) == _coreSceneName) yield break;

        AsyncOperation unloadOp = UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync(sceneNumber);
        while (!unloadOp.isDone) yield return null;
    ...
    private IEnumerator TransitionRoutine(string newSceneName)
    {
        ...
        {
            if (scene != _coreSceneName && scene != _loadingSceneName)
            {
    ...
    private IEnumerator LoadSceneInternal(int sceneNumber)
    {
        ...
        AsyncOperation loadOp = UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(sceneNumber, LoadSceneMode.Additive);
        loadOp.allowSceneActivation = false;
